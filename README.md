...
- **分页渲染**：SELECT 结果每页 20 行，翻页不重跑查询；支持页码输入框直接跳转（回车或「跳转」按钮，越界自动忽略）；结果切换带淡入动画
...
> 页码输入框聚焦时 `F5` 不触发执行，避免跳转页码时误跑查询。

## 语法高亮配色

> 采用 **Slate Dawn** 浅色主题（设计稿见 [docs/superpowers/specs/](docs/superpowers/specs/2026-05-31-slate-dawn-theme-design.md)）

| 元素 | 样式 |
|---|---|
| 关键字（`SELECT`、`WHERE` 等） | 靛蓝 `#5B6CF4`，加粗 |
| 数字 | 深绿 `#1E7A1E` |
| 运算符（`=`、`<`、`>`、`!=` 等） | 深紫 `#7B3FA0` |
| 表名（四张 CSV 表） | 琥珀棕 `#B87333` |
| 列名（已知 Schema 列） | 海蓝 `#0076C8` |
| 字符串字面量 | 琥珀棕 `#B87333` |
| 注释（`--` 行注释） | 灰蓝 `#8B90A8`，斜体 |
| 出错 token | 深红 `#D32F2F` 波浪下划线 |
